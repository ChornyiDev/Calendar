...
function init() {
  ...
  if (savedStartDay) {
    weekStartDay = savedStartDay === 'auto' ? detectStartDay() : parseInt(savedStartDay);
...
function updateTimezoneDisplay() {
  // Can add visual clues if needed, but selector shows value.
}
...
function convertTime(dateStr, start, end) {
  ...
  const formatInUserTz = (d) => new Intl.DateTimeFormat('en-GB', { 
  ...
  const userDateStr = new Intl.DateTimeFormat('en-CA', { // YYYY-MM-DD
  ...
  return { start: formatInUserTz(startDate), end: formatInUserTz(endDate), diffDate };
}
...
function selectDate(cell, dateStr, status) {
  ...
  const prettyDate = new Intl.DateTimeFormat('en-US', { weekday: 'long', month: 'long', day: 'numeric' }).format(dateObj);

  let html = `<div class="slot-title">${prettyDate}</div>`;

  if (status.type === 'off' || status.type === 'off-special') {
    const label = status.type === 'off-special' ? 'Holiday / Day Off' : 'Day Off';
